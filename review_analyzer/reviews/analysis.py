...
def generate_recommendation_with_gemini(praise, complaints):
    ...
    The recommendation is returned as a list of points.
    """
    if not gemini_model:
    ...
    prompt_parts.append("\nActionable Recommendation (Provide as a concise, bulleted list where each point starts with '*'):")
    prompt = "\n".join(prompt_parts)
    ...
    try:
        ...
        if not cleaned_points:
            return [recommendation_text] if recommendation_text else ["No specific recommendation could be generated."]
